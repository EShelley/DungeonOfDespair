...
public:
	...
	Actor(boost::uuids::uuid ID, std::string Name, std::string ShortDescription, std::string DetailedDescription,int currenthp, int maxhp, bool isLive) : Entity(ID, "BaseActor", Name, ShortDescription, DetailedDescription),currentHP(currenthp), maxHP(maxhp),alive(isLive) {};
	~Actor() {};
